...
_MESSAGE_RULES: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"^research generated by Claude$"), "Claude đã tạo bản nghiên cứu"),
    ...
    (re.compile(r"^outline generated by Claude$"), "Claude đã tạo dàn ý"),
    (re.compile(r"^script scaffold written.*$"),
     "đã tạo khung kịch bản (đặt approved=true trước khi lồng tiếng)"),
    (re.compile(r"^script generated by Claude.*$"),
     "Claude đã tạo kịch bản — cần duyệt trước khi lồng tiếng"),
    (re.compile(r"^scene_plan written \((\d+) clips, ([\d.]+) min total, (\d+) source_clips resolved\)$"),
     r"đã tạo kế hoạch cảnh (\1 shot hình, tổng \2 phút, khớp \3 cảnh nguồn)"),
    (re.compile(r"^scene_plan generated by Claude \((\d+) clips, ([\d.]+) min total, (\d+) source_clips resolved\)$"),
     r"Claude đã chọn kế hoạch cảnh (\1 shot hình, tổng \2 phút, khớp \3 cảnh nguồn)"),
    (re.compile(r"^synthesized narration\.mp3 from (\d+) sections$"),
     r"đã tổng hợp narration.mp3 từ \1 phần lời dẫn"),
    (re.compile(r"^aligned (\d+) cues within narration bounds \(dropped (\d+)\) from voice$"),
     r"đã căn \1 phụ đề theo lời dẫn TTS (bỏ \2)"),
    ...
    (re.compile(r"^rendered (\d+) clips to final\.mp4$"),
     r"đã kết xuất \1 shot hình vào final.mp4"),
    ...
    (re.compile(r"^generated (\d+) thumbnail candidates$"),
     r"đã tạo \1 ảnh bìa ứng viên"),
    (re.compile(r"^thumbnail source has no positive duration$"),
     "nguồn ảnh bìa không có thời lượng hợp lệ"),
    (re.compile(r"^source video/final\.mp4 missing - run render before thumbnail$"),
     "thiếu video nguồn/final.mp4 — chạy bước render trước ảnh bìa"),
